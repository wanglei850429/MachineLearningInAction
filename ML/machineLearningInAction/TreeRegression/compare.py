...
#     ones = np.ones(m)
#     X_test1 = np.insert(dataset_test[:,:-1],0,ones,axis=1)
#     y_test1 = dataset_test[:, -1].A.flatten()
...
#     corrcoef_lr = get_corrcoef(y_test, y_lr)
    corrcoef_lr = np.corrcoef(y_lr, y_test, rowvar=0)[0,1]
...
#     corrcoef_tree = get_corrcoef(np.array(y_tree), y_test)
    corrcoef_tree = np.corrcoef(np.array(y_tree), y_test, rowvar=0)[0,1]
